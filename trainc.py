...
def hybridCameras(realcameras,args):
    beforeCamera=realcameras[0]
    hybridCamera=[]
    ...
    for index in range(1,len(realcameras)):
        ...
        hybrid.T=T
        hybrid.colmap_id=-1
        hybridCamera.append(hybrid)
        hybridCamera.append(realcameras[index])
        beforeCamera=realcameras[index]
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        if not viewpoint_stack:
            if not reg_kick_on:
                ...
                viewpoint_stack = hybridCameras(scene.getTrainCameras().copy(),args)
        viewpoint_cam = viewpoint_stack.pop(randint(0, len(viewpoint_stack)-1))

        voxel_visible_mask = prefilter_voxel(viewpoint_cam, gaussians, pipe,background)
        retain_grad = (iteration < opt.update_until and iteration >= 0)
        render_pkg = render(viewpoint_cam, gaussians, pipe, background, visible_mask=voxel_visible_mask, retain_grad=retain_grad, require_coord = require_coord and reg_kick_on, require_depth = require_depth and reg_kick_on)
        ...
        if viewpoint_cam.colmap_id!=-1:
            ...
            rgb_loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * ssim_loss + 0.01*scaling_reg   
            if reg_kick_on:
        ...
        with torch.no_grad():
            ...
            depth_normal_loss_for_log = 0.4 * depth_normal_loss.item() * lambda_depth_normal + 0.6 * depth_normal_loss_for_log

            if iteration % 10 == 0:
                progress_bar.set_postfix({"Loss": f"{ema_loss_for_log:.{4}f}", "Depth-normal Loss": f"{depth_normal_loss_for_log:.{4}f}"})

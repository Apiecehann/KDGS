	...
	const bool debug)
{
  ...
  torch::Tensor out_depth = torch::full({1, H, W}, 0.0f, float_opts);
  torch::Tensor out_mdepth = torch::full({1, H, W}, 0.0f, float_opts);
  torch::Tensor out_coord = torch::full({3, H, W}, 0.0f, float_opts);
  torch::Tensor out_mcoord = torch::full({3, H, W}, 0.0f, float_opts);
  torch::Tensor out_alpha = torch::full({1, H, W}, 0.0f, float_opts);
  torch::Tensor out_normal = torch::full({3, H, W}, 0.0f, float_opts);
  ...
  return std::make_tuple(rendered, out_color, radii, out_coord, out_mcoord, out_alpha, out_normal, out_depth, out_mdepth,  geomBuffer, binningBuffer, imgBuffer);
}

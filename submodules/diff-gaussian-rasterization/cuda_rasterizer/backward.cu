	...
	bool require_depth)
{
	...
	renderCUDA<NUM_CHANNELS, template_coord, template_depth, template_normal> << <grid, block >> >(ranges,point_list,W, H,bg_color,view_points,means2D,\
	conic_opacity,colors,depths,ts,camera_planes,ray_planes,alphas,normals,accum_coord,accum_depth,normal_length,n_contrib,dL_dpixels,dL_dpixel_coords,\
	dL_dpixel_mcoords,dL_dpixel_depth,dL_dpixel_mdepth,dL_dalphas,dL_dpixel_normals,normalmap,focal_x, focal_y,dL_dmean3D,dL_dmean2D,dL_dconic2D,\
	dL_dopacity,dL_dcolors,dL_dts,dL_dcamera_planes,dL_dray_planes,dL_dnormals)
